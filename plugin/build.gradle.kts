...
val esphomeDeviceBuilderRef: Provider<String> = providers.gradleProperty("esphomeDeviceBuilderRef")
    .orElse("292ed4f293a0f8a9163e954608baaf47d617be4b") // ESPHome schema 2026.8.2
...
// --- lvgl catalog overlay (permanent) ---------------------------------------
//
// device-builder deliberately ships `lvgl` with zero config_entries — its
// recursive widget tree flattens to ~14 MB, so upstream drops the body rather
// than carry dead weight its own dashboard never renders (an intentional
// policy, not a bug: `_YAML_ONLY_COMPONENT_IDS` in device-builder's
// script/sync_components.py, see PR esphome/device-builder#1510). It will
// never come back from a ref bump, so `plugin/catalog-overlay/` is permanent,
// not a stopgap — regenerate it (see below) whenever the pin moves far enough
// that lvgl's config options have drifted.
//
// `components/lvgl.json` here is regenerated by: taking a device-builder
// checkout from just *before* #1510 (so its sync script still emits a real
// lvgl body) and running `script/sync_components.py --version <esphome
// version> --limit-component lvgl`, then dropping the `widgets` entry (the
// recursive widget tree — ~14 MB by itself, and irrelevant here: completion
// for `lvgl:` at any depth is routed to the raw-language-schema converter in
// `EsphomeLangSchemaService`/`plugin/src/main/resources/esphome/langschema/`
// instead, which handles the recursion compactly). What's left (~190
// top-level lvgl config fields) is still real, used data — hover docs and
// id-reference navigation for fields like `displays:` go through this
// flattened catalog body, not the language schema. Re-add
// `"references_component": "display"` on the `displays` entry by hand after
// regenerating: lvgl declares it via a custom validator the schema dumper
// can't see through, so the sync always emits a bare `id` with no target
// class. Also fix the component metadata (`name`/`description`/`docs_url`,
// drop `image_url`) — the raw sync mislabels `lvgl` from an id collision with
// its `lvgl.number` platform row.
//
// The automations index needs no such overlay: device-builder's own
// `automations.index.json` already carries lvgl's on-obj triggers correctly
// (verified against the current vendored output), so only the component body
// and the index relabel below are still needed.
