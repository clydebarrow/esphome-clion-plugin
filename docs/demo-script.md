...
### Segment 3 — Hover docs incl. domain hover (~20s)
...
### Segment 5 — Navigation (~35s) — the money shot
...
### Segment 6 — Lambda highlighting + checks (~30s)
...
### Segment 7 — secrets.yaml masking (~20s)
...
### Segment 8 — Device tool window (~35s) *(needs a live device; optional)*
- Right-click in `demo.yaml` → **Open ESPHome Device Window** (or click the stripe). It auto-connects.
...
### Segment 9 — Run it (~20s) *(optional)*
- Right-click the config → create/run a **Run** configuration (compile + upload + logs) → ANSI-colored ESPHome output in the Run console.
...
- For Segment 8 without a device: still show the window opening, host/key pre-fill, and the "connecting…" state — entities need a live device.
